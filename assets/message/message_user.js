...
$('.upload_attachmentfile').on('change', function(){
	...
	$.ajax({
                url: base_url+'user/chat-attachment/upload', 
                dataType: 'json',  
                cache: false,
                contentType: false,
                processData: false,
...
$('.ClearChat').on('click', function(){
       var receiver_id = $('#ReciverId_txt').val();
  	 			$.ajax({
						  //dataType : "json",
  						  url: base_url+'user/chat-clear?receiver_id='+receiver_id,
...
function DisplayMessage(message){
	...
	var Sender_ProfilePic = $('#Sender_ProfilePic').val();
		...
		var str = '<div class="convo-box pull-right">';
				str+='<div class="convo-area pull-right">';
				 str+='<div class="convo-message"><p>'+message ;
				 str+='</p><span>'+Sender_Name+'</span></div>'; //23 Jan 2:05 pm
				 str+='<div class="convo-img"><img class="img-fluid rounded" src="'+base_url+'public/uploads/users/'+Sender_ProfilePic+'" alt="image">';
				 str+='</div>'
				 str+='</div></div>';
		$('#dumppy').append(str);
}
		...
		//console.log(message);
 		DisplayMessage(messageTxt);
				...
				$.ajax({
						  ...
						  url: base_url+'user/send-message',
						  success:function(data)
						  {
  							GetChatHistory(receiver_id)		 
						  },
