...
$('#myLink').on('click', function(){ 
    ...
    var userid = $('#from_id').val();

    $.ajax({
        type: "POST",
        url: base_url+'user/create',
        data: "userid="+userid+"&to_id="+to_id+"&message="+encodeURIComponent(chat_input), 
        dataType: 'json',
...
$(document).on('click','.user-selected-for-chat',function(e){
  e.preventDefault();
    ...
    $.ajax({
        type: "POST",
        url: base_url+'user/ajax_get_user_by_id', 
        data: "id="+id,
        dataType: "json",
        success: function(result) 
        {	
          ...
          if(result['error'] == false){
            $("#to_id").val(result['data'].id);
              ...
              '<img src="'+base_url+'public/uploads/admin/'+result['data'].imagelocation+'" alt="'+result['data'].name+'" width="40" height="40">'+
